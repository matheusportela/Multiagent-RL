...
class RandomWorldAgent(pymas.Agent):
    ...
    def __init__(self):
        ...
        self.learner = learning.QLearning(initial_state=self.state,
            learning_rate=1, discount_factor=1, actions=self.actions)
    ...
    def on_run(self):
        action = self.learner.act(self.state)
    ...
    def on_receive_message(self, message):
        if (message.sender == WindyWaterAgent.simulator_id):
            if message.data == 'stop':
                ...
                self.state = message.data['coordinates']
                self.learner.learn(self.state, self.last_action, message.data['reward'])
    ...
    * * * * * * * * * *
    ...
    def calculate_state(self, action):
        ...
        self.agent_coordinates = (
            min(max(self.agent_coordinates[0] + action[0] + wind_action[0], 0), self.rows - 1),
            min(max(self.agent_coordinates[1] + action[1] + wind_action[1], 0), self.cols - 1),
        )
    ...
    def on_run(self):
        ...
        if self.is_episode_finished():
            print 'Episode %d finished' % self.current_episode
