...
class MessengerBase(object):
    """Base class for communicating messages."""

    def receive(self):
        """Requests a message and returns it."""
        raise NotImplementedError('Messenger must receive message,')
...
class ZMQTCPMessenger(MessengerBase):
    """Base class for simple communicating messages through zmq."""
    def __init__(self, binding):
        super(ZMQTCPMessenger, self).__init__()
        self.__configure_socket__(binding)

    def __configure_socket__(self, binding):
        raise NotImplementedError('ZMQTCPMessenger must configure socket.')
...
class TCPServer(ZMQTCPMessenger):
...
class TCPClient(ZMQTCPMessenger):
    ...
    def __configure_socket__(self, connection):
        context = zmq.Context()
